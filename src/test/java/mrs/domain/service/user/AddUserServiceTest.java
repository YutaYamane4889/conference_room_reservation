...
public class AddUserServiceTest {
	...
	@Test
	public void _1件追加で正常に追加されるか検証() {
		User info = new User();
		info.setUserId("dddd");
		info.setFirstName("テスト4");
		info.setLastName("テスト4");
		info.setPassword("pass");
		info.setRoleName(RoleName.ADMIN);
	...
	@Test
	public void _1件追加で正常内容も追加されるか検証() {
		User info = new User();
		info.setUserId("dddd");
		info.setFirstName("テスト4");
		info.setLastName("テスト4");
		...
		service.addUser(info);
		List<User> data = repository.findAll();
		assertThat(data.get(3).getUserId(),is("dddd"));
		assertThat(data.get(3).getFirstName(),is("テスト4"));
		assertThat(data.get(3).getLastName(),is("テスト4"));
		assertThat(passwordEncoder.matches("pass", data.get(3).getPassword()),is(true));
		assertThat(data.get(3).getRoleName(),is(RoleName.ADMIN));
	}

	@Test
	public void _1件追加でほかに影響がないか検証() {
		User info = new User();
		info.setUserId("dddd");
		info.setFirstName("テスト4");
		info.setLastName("テスト4");
	...
	@Test
	public void __1件追加でほかに影響がないか検証() {
		User info = new User();
		info.setUserId("dddd");
		info.setFirstName("テスト4");
		info.setLastName("テスト4");
